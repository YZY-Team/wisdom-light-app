...
export default function PrimaryCategory({ title, isActive, onPress }: PrimaryCategoryProps) {
  return (
    <Pressable
      className={` h-10  items-center ${!isActive && 'border-b-2 border-transparent hover:bg-gray-50'}`}
      onPress={onPress}>
      {isActive ? (
        <LinearGradient
