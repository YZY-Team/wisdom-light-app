...
import { View, Text, TouchableOpacity, StyleSheet, Dimensions, GestureResponderEvent } from 'react-native';
...
const StudentsManagementWithFlashList = () => {
  ...
  const handleLetterSelect = (letter: string) => {
    ...
    const index = flattenedData.findIndex((item) => item.type === 'header' && item.title === letter);
  ...
  const findLetterAtPosition = (touchY: number) => {
    ...
    const index = Math.min(
      Math.max(Math.floor(relativeY / letterHeight), 0),
      ALPHABET.length - 1
    );

    // 找到对应的字母
    const letter = ALPHABET[index];
    console.log('触摸位置:', touchY, '相对位置:', relativeY, '字母索引:', index, '选中字母:', letter);
  ...
  const renderLetterBubble = () => {
    ...
    return (
      ...
      >
        <View className="w-16 h-16 rounded-full bg-blue-500 shadow-lg items-center justify-center">
          <Text className="text-white text-2xl font-bold">{activeLetter}</Text>
        </View>
      </View>
    );
  };
  ...
  const renderLetter = (letter: string) => (
    <TouchableOpacity
      key={letter}
      className="h-5 justify-center items-center"
      onPress={() => handleLetterPress(letter)}
    >
      <Text
        className={`text-[12px] ${activeLetter === letter ? 'text-white font-bold' : 'text-gray-300'}`}
      >
        {letter}
      </Text>
    </TouchableOpacity>
  ...
  const renderItem = ({ item }: { item: Item }) => {
    ...
    return (
      <View className="flex-col flex-wrap gap-3 mb-4">
  ...
  return (
    <View className="flex-1 bg-white flex-row">
      ...
      <View className="relative">
        ...
        {/* 字母索引列表 */}
        <View
          className="w-12 h-full justify-center items-center "
        >
          <View ref={alphabetContainerRef} className=" justify-center  items-center" onLayout={() => {
            // 布局完成时测量容器位置
            setTimeout(() => {
              alphabetContainerRef.current?.measure((x, y, width, height, pageX, pageY) => {
