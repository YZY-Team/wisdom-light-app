...
export default function WeeklyDeclarationItem({ declaration, onUpdateDeclaration, readOnly = false }: WeeklyDeclarationItemProps) {
  ...
  // 七天的行动计划数据 (周一到周日)
  const [dailyPlans, setDailyPlans] = useState<ActionPlan[]>([
  ...
  useEffect(() => {
    ...
    // 如果declaration中包含actionPlans数据，则更新dailyPlans
    if (declaration.actionPlans && declaration.actionPlans.length > 0) {
      const plans: ActionPlan[] = Array(7).fill({ achievementPlan: '', completion: '' });
      
      declaration.actionPlans.forEach(plan => {
        if (plan.dayOfWeek >= 0 && plan.dayOfWeek < 7) {
          plans[plan.dayOfWeek] = {
            achievementPlan: plan.achievementPlan || '',
  ...
  const autoSave = async () => {
    ...
    try {
      ...
      // 准备actionPlans数据
      const actionPlans = dailyPlans.map((plan, index) => ({
        dayOfWeek: index,
        achievementPlan: plan.achievementPlan || '',
        completion: plan.completion || '',
      }));
      
      // 更新包含actionPlans的localDeclaration
      ...
      if (response.code === 200) {
        // 更新成功，通知父组件更新数据
        onUpdateDeclaration(response.data || updatedDeclaration);
      }
    } catch (error) {
